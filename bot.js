...
bot.on(/^[^/].*/, msg => {
    if (globalConfig.mode) {
        ...
        var cutOf = text.slice(0, 4);
        var relpyFormat = [`${text}个几把`, `你慢慢${text}``${text}个屁`, `不许${cutOf}`, "你说你🐴呢？", "有一说一确实",`${text}`,`cnm`,`给${exp.random_ye()}整乐了`,`?`,`爬爬爬`,`NM$L`,`给${exp.random_ye()}少说两句又不会死`];
